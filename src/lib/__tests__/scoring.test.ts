...
import { calculateMatch, hasActiveFilters, countMatches, MATCH_COLORS } from "../scoring";
...
describe("calculateMatch", () => {
  ...
  it("season match: month in bestMonths returns match", () => {
    expect(
      calculateMatch(makeCountry({ bestMonths: [6, 7, 8] }), { monthFrom: 7, monthTo: 7 }),
    ).toBe("great");
  });
  ...
  it("season match: month not in bestMonths returns good — other 2 criteria pass (no filter)", () => {
    ...
    expect(
      calculateMatch(makeCountry({ bestMonths: [6, 7, 8] }), { monthFrom: 1, monthTo: 1 }),
    ).toBe("good");
  });
  ...
  it("duration match: exact min boundary matches (overlap)", () => {
    ...
    expect(
      calculateMatch(makeCountry({ recommendedDaysMin: 7, recommendedDaysMax: 14 }), filters),
    ).toBe("great");
  });
  ...
  it("budget match: range includes mid", () => {
    ...
    expect(
      calculateMatch(makeCountry({ dailyBudgetMid: 40 }), { budgetMin: 20, budgetMax: 100 }),
    ).toBe("great");
  });
