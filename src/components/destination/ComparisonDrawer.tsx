...
export function ComparisonDrawer({ codes, filters, onRemove, onClearAll }: Props) {
  ...
  const bestBudget =
    validCountries.length > 1 ? Math.min(...validCountries.map((c) => c!.dailyBudgetLow)) : null;
  const bestSafety =
    validCountries.length > 1 ? Math.max(...validCountries.map((c) => c!.safetyScore)) : null;
  ...
  return (
    ...
    >
      ...
      {/* Columns */}
      <div
        className={`grid ${cols} divide-y divide-white/10 sm:divide-y-0 sm:divide-x max-h-[30vh] sm:max-h-[38vh] overflow-y-auto scrollbar-none`}
      >
...
function StatLine({ label, value, best }: { label: string; value: string; best?: boolean }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span
        className={`text-xs font-semibold tabular-nums ${best ? "text-green-400" : "text-white"}`}
      >
        {value}
      </span>
    </div>
